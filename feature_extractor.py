...
# 특징 추출 로직을 변경할 때마다 이 값을 올려주세요.
# model_manager.py의 특징 캐시는 이 버전이 다르면 캐시를 무시하고 다시 추출합니다
# (버전이 다른 이전 특징과 새 특징이 섞여 차원이 안 맞는 오류가 나는 것을 방지).
FEATURE_VERSION = 1
...
def augment_variants(img_bgr):
    ...
    좌우/상하 방향, 밝기, 대비, 초점, 센서 잡음이 조금씩 달라질 수 있으므로,
    그런 변형들을 미리 만들어 함께 학습시켜 신뢰율(일반화 성능)을 높입니다.
    (원본 1장당 총 10가지 변형: 원본, 좌우반전, 상하반전, 회전±7도, 밝게/어둡게,
     가우시안 노이즈, 약한 블러, 대비 변화)
    """
    variants = [img_bgr]
    ...
    # 대비(contrast) 변화 (평균 밝기는 유지한 채 대비만 조절)
    alpha_c = 1.25
    beta_c = 128 * (1 - alpha_c)
    contrast_up = cv2.convertScaleAbs(img_bgr, alpha=alpha_c, beta=beta_c)
